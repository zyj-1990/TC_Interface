...
public class IntegralRecord extends ZhaoyanjiConfig{
    ...
    @Test
    public void integralRecord() throws Exception {
        ...
        paras.add(new Parameter("pagesize",15));
        paras.add(new Parameter("page",1));

        Http httpRequest = new Http("get", paras, headers, null);
        JSONObject res = HttpRequest.sendRequest(httpRequest, host, "/index/integralRecord");
        String err_msg = CommonApi.get_ErrorMsg(res);
        System.out.println(res);
        Assert.assertEquals(err_msg, "success", "获取积分接口调用不成功");
    }
